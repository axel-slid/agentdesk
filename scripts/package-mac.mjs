...
const productName = packageJson.productName || "AgentDesk";
const bundleId = "com.agentdesk.latex";
...
const sourceIcon = path.join(appRoot, "assets", "AgentDesk.icns");
...
function writeMainPlist() {
  ...
  safeSetPlist(plistPath, "CFBundleIconFile", "AgentDesk.icns");
  safeSetPlist(plistPath, "LSApplicationCategoryType", "public.app-category.developer-tools");
  safeSetPlist(plistPath, "NSCameraUsageDescription", "AgentDesk does not use the camera.");
  safeSetPlist(plistPath, "NSMicrophoneUsageDescription", "AgentDesk does not use the microphone.");
}
...
async function main() {
  ...
  await fsp.copyFile(sourceIcon, path.join(resourcesDir, "AgentDesk.icns"));
